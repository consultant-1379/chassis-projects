...
public class ImsiprefixProfileGetExecutor extends Executor {
  ...
  private static ImsiprefixProfileGetExecutor instance;
  ...
  protected ExecutionResult execute(NFMessage request) {
    ImsiprefixProfileGetRequest getRequest = request.getRequest().getGetRequest()
        .getImsiprefixProfileGetRequest();
    return ClientCacheService.getInstance().getAllByID(Code.IMSIPREFIXPROFILE_INDICE,
        getSearchImsiprefixList(getRequest.getSearchImsi()));
  }
