...
public class CacheNFProfileGetExecutor extends Executor {
  ...
  private static CacheNFProfileGetExecutor instance;
  ...
  protected ExecutionResult execute(NFMessage request) {
    CacheNFProfileGetRequest getRequest = request.getRequest().getGetRequest()
        .getCacheNfProfileGetRequest();
    return ClientCacheService.getInstance()
        .getByID(Code.CACHENFPROFILE_INDICE, getRequest.getCacheNfInstanceId());
  }
}
