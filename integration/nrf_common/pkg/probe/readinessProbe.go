...
func ReadinessProbe_Handler(rw http.ResponseWriter, req *http.Request) {
	if isFirstFlag {
		monitorKVDBInitJobDone()
		isFirstFlag = false
	}
	if !isKVDBInitDone {
		rw.WriteHeader(http.StatusInternalServerError)
		log.Warningf("kvdb init job is not done, readinessProbe result %d", http.StatusInternalServerError)
		return
	}
	ready := isDBReady()
...
// monitorKVDBInitJobDone is to monitor the kvdb init job state
func monitorKVDBInitJobDone() {
	ticker := time.NewTicker(time.Second * 1)
	jobName := os.Getenv("KVDB_INIT_JOB")
	go func() {
		for {
			select {
			case <-ticker.C:
				isKVDBInitDone = k8sclient.GetK8sAPIClient().GetIsJobCompleted(jobName)
				if isKVDBInitDone {
					log.Debugf("job %s is complete", jobName)
					return
				}
			}
		}
...
func isNfProfileQuerySuccess() bool {
	...
	go func() {
		...
		for i = 0; i < warmUpNum; i++ {
			getReq := &dbproxy.QueryRequest{
				RegionName: "ericsson-nrf-nfprofiles",
				Query:      []string{"test123123213213-12321"},
			}

			queryRes, err := dbmgmt.QueryWithKey(getReq)
			if err == nil && (queryRes.GetCode() == dbmgmt.DbDataNotExist || queryRes.GetCode() == dbmgmt.DbGetSuccess) {
				dbReady = true
			} else {
				log.Warning("isNfProfileQuerySuccess failed")
		...
		log.Debug("instanceId warm up success")
	}()
