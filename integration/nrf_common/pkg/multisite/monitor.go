...
func (m *Monitor) updateSiteStatus() {
	...
	if m.isActiveSite {
		instanceID := cm.GetNRFNFProfile().InstanceID
...
func (m *Monitor) IsActiveSite() bool {
	...
	nrfCommon := cm.GetNRFCommon()
	log.Debug(nrfCommon.GeoRed)
	if cm.ServiceName == cm.ManagementWorkMode && nrfCommon.GeoRed.KeepManagementService {
		return true
	}

	if cm.ServiceName == cm.DiscoveryWorkMode && nrfCommon.GeoRed.KeepDiscoveryService {
		return true
	}

	if cm.ServiceName == cm.NotificationWorkMode && nrfCommon.GeoRed.KeepManagementService {
		return true
	}

	if cm.ServiceName == cm.ProvsionWorkMode && nrfCommon.GeoRed.KeepManagementService {
		return true
	}
...
func getLeader(activeSites []StatusInfo) (string, float32) {
	...
	for i := range activeSites {
		...
		if cm.ServiceName == cm.ManagementWorkMode {
			...
			if activeSites[i].Fqdn != "" {
				fm.ClearNRFReplicationFailureAlarm(activeSites[i].Fqdn)
			} else {
				fm.ClearNRFReplicationFailureAlarm(activeSites[i].InstanceID)
			}
		}
	}
...
func handleDownSites(downSites []StatusInfo) {
	for i := range downSites {
		...
		if downSites[i].InstanceID == nfProfile.InstanceID {
			continue
		}
