...
func updateWitnessInDB(instanceId, fqdn string) {
	...
	resp, err := dbmgmt.Insert(region, "witness", data)
	if err != nil {
		log.Error(err)
	} else {
		if resp.GetCode() != dbmgmt.DbPutSuccess {
			log.Errorf("Fail to insert data to DB, error code %d", resp.Code)
		}
	}
}
...
func removeWitnessInDB(instanceId string) {
	...
	_, err := dbmgmt.Remove(region, keys)
        if err != nil {
                log.Error(err)
        }
}
...
func (m *Manager) IsWitness(instanceId, fqdn string) bool {
	...
	switch nrfCommon.GeoRed.WitnessNF.IdentityType {
	case "nf-instance-id":
		if nrfCommon.GeoRed.WitnessNF.IdentityValue == instanceId {
			m.witnessID = instanceId
			m.witnessFQDN = fqdn
			return true
		}
	case "nf-fqdn":
		if nrfCommon.GeoRed.WitnessNF.IdentityValue == fqdn {
			m.witnessID = instanceId
			m.witnessFQDN = fqdn
			return true
		}
