...
func TestTNfProfileParseConf(t *testing.T) {
	...
	if len(GetNRFNFServices().ManagementNfServices) != 1 || len(GetNRFNFServices().DiscoveryNfServices) != 1 {
...
func TestTNfProfileToUpper(t *testing.T) {
	...
	if nfProfile.Type != "NRF" || nfProfile.RequestedStatus != "REGISTERED" {
