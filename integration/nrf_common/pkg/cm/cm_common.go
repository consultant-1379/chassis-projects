...
type TCommon struct {
	RegionNrf *TRegionNrf `json:"region-nrf"`
	PlmnNrf   *TPlmnNrf   `json:"plmn-nrf"`
	GeoRed    *TGeoRed    `json:"geo-red"`
}

// TRegionNrf is configuration of region nrf
type TRegionNrf struct {
	NextHop *TNextHopNrf `json:"next-hop"`
}

// TNextHopNrf is configuration of plmn nrf used by region nrf
type TNextHopNrf struct {
	Mode string     `json:"mode"`
	Site []TSiteNRF `json:"pool"`
}

// TSiteNRF is configuration of nrf pool
type TSiteNRF struct {
	ID      string        `json:"id"`
...
// TPlmnNrf is configuration of plmn nrf
...
type TNfProfile struct {
	Service                 []TNfService `json:"service"`
	InstanceID              string       `json:"instance-id"`
	Type                    string       `json:"type",omitempty`
	RequestedStatus         string       `json:"requested-status,omitempty"`
...
type TNfService struct {
	InstanceID                      string                             `json:"instance-id"`
	Name                            string                             `json:"name"`
	Version                         []TVersion                         `json:"version,omitempty"`
	Scheme                          string                             `json:"scheme"`
 ...
 * -------------------------------------------- struct list for provisioning-service ---------------------------
 */
...
type TManagementService struct {
	...
	TrafficRateLimitPerInstance int         `json:"traffic-rate-limit-per-instance"`
}
...
type TDiscoveryService struct {
	...
	LocalCache          *TLocalCache          `json:"local-cache"`
	OverloadRedirection *TOverloadRedirection `json:"overload-redirection"`
}

// TLocalCache is configuration of discovery local cache
...
// TOverloadRedirection is configuration of discovery overload redirection
type TOverloadRedirection struct {
	Enabled        bool     `json:"enabled"`
	Mode           string   `json:"mode"`
	RedirectionURL []string `json:"redirection-url"`
}
...
type TNrfPolicy struct {
	...
	MessagePriorityPolicy *TMessagePriorityPolicy      `json:"sbi-message-priority-policy"`
}
...
// TMessagePriorityPolicy is configuration of message priority policy
type TMessagePriorityPolicy struct {
	MediumStart     *int `json:"priority-group-medium-start"`
	LowStart        *int `json:"priority-group-low-start"`
	DefaultPriority *int `json:"default-message-priority"`
}
...
var (
	...
	// IPStackMode is the IP version of cluster
	IPStackMode string
)
...
// SetIPStackMode is to set the IP stack version(ipv4/ipv6) of cluster
func SetIPStackMode(mode string) {
	IPStackMode = mode
}
