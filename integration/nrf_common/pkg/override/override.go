...
// AppendProvisionInfo is for append ProvisionInfo for NFProfile
func AppendProvisionInfo(nfProfileBody *[]byte, overrideExist bool, overrideInfo *[]byte, provFlag int64) bool {
	if nfProfileBody == nil {
		log.Errorf("AppendProvisionInfo ptr is nil")
		return false
	}
	...
	if err != nil {
		log.Errorf("AppendProvisionInfo Unmarshal nfProfile failure: %s", err.Error())
		return false
	}
	provInfo := new(nrfschema.TProvInfo)
	if provFlag == constvalue.Cmode_NFRegistered {
		var overrideAttrList = make([]string, 0)
		if overrideExist {
			overrideAttrList = getOverrideIAttrList(overrideInfo)
		}
		provInfo.CreateMode = constvalue.CMODE_NF_REGISTERED
		provInfo.OverrideAttrList = overrideAttrList

	} else if provFlag == constvalue.Cmode_Provisioned {
		provInfo.CreateMode = constvalue.CMODE_PROVISIONED
	} else {
		log.Errorf("AppendProvisionInfo provisioned flag is wrong: %d", provFlag)
		return false
	}
	nfProfile.ProvisionInfo = provInfo

	*nfProfileBody, err = json.Marshal(nfProfile)
	if err != nil {
		log.Errorf("AppendProvisionInfo Marshal nfProfile error: %s", err.Error())
		return false
	}
	return true
}

func getOverrideIAttrList(overrideInfo *[]byte) []string {
	overrideList := make([]nrfschema.OverrideInfo, 0)
	var overrideAttrList = make([]string, 0)
	err := json.Unmarshal(*overrideInfo, &overrideList)
	if err != nil {
		log.Errorf("getOverrideIAttrList Unmarshal overrideInfo failure: %s", err.Error())
		return overrideAttrList
	}
	for _, item := range overrideList {
		overrideAttrList = append(overrideAttrList, item.Path)
	}
