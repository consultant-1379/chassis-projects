...
func NewWorkEngineManager(totalCounterName, unprocessedCounterName string) *WorkEngineManager {
	manager := new(WorkEngineManager)

	manager.Status = Normal
	manager.ProcessedRequestNumber = 0
	manager.DeniedRequestNumber = 0
	manager.ProcessRequestWorkerNumber = 32
	manager.ProcessRequestQueueCapacity = 6400
	manager.DeniedRequestWorkerNumber = 8
	manager.DeniedRequestQueueCapacity = 10240
	manager.StatisticsQueueCapacity = 4096

	manager.TotalCounterName = totalCounterName
	manager.DeniedCounterName = unprocessedCounterName
...
func (manager *WorkEngineManager) SetOverloadControlLevel(level int64) {
...
func (manager *WorkEngineManager) SetIdleRecoverRatio(ratio int64) {
...
func (manager *WorkEngineManager) SetProcessRequestWorkerNumber(number int) {
	if number > 0 {
		manager.ProcessRequestWorkerNumber = number
	}
}

func (manager *WorkEngineManager) SetProcessRequestQueueCapacity(capacity int) {
	if capacity > 0 {
		manager.ProcessRequestQueueCapacity = capacity
	}
}
...
func (manager *WorkEngineManager) SetOverloadRedirectEnabled(enabled bool) {
	manager.OverloadRedirectEnabled = enabled
}

func (manager *WorkEngineManager) SetOverloadRedirectAddr(addr string) {
	manager.OverloadRedirectAddr = addr
}
...
func (manager *WorkEngineManager) Start() {
	for _, engine := range manager.WorkEngines {
		manager.WorkEnginePrioritys = append(manager.WorkEnginePrioritys, engine.GroupPriority)
	}
	sort.Ints(manager.WorkEnginePrioritys)
	...
	manager.ProcessRequests = make(chan Task, manager.ProcessRequestQueueCapacity)
	...
	for i := 0; i < manager.ProcessRequestWorkerNumber; i++ {
		go manager.HandleProcessRequest()
	}
...
func (manager *WorkEngineManager) GetRequestStatistics(count map[int]uint64) string {
	statistics := ""
	for _, priority := range manager.WorkEnginePrioritys {
		if value, ok := count[priority]; ok {
			statistics += fmt.Sprintf("%2d:%4d,", priority, value)
		}
	}
	return statistics
}
...
func (manager *WorkEngineManager) GetPassRate() string {
	...
	for _, engine := range manager.WorkEngines {
		...
		passRate += fmt.Sprintf("[%2d,%2d]:%5.2f,", engine.MessagePriorityStart, engine.MessagePriorityEnd, rate)
	}
	return passRate
}

func (manager *WorkEngineManager) GetPendingRequestNumber() int {
	return len(manager.ProcessRequests)
}

func (manager *WorkEngineManager) GetTotalConfigLevel() int64 {
	var total int64 = 0
...
func (manager *WorkEngineManager) TriggerOverload(count map[int]uint64, ratio float64, window uint64) {
	...
	for i := len(manager.WorkEngines) - 1; i >= 0; i-- {
		...
		if count[groupPriority] != 0 {
			...
			if step <= capacity {
				manager.WorkEngines[i].CurrentOverloadLevel -= int64(step / base * float64(manager.OverloadControlLevel))
				break
			} else {
...
func (manager *WorkEngineManager) DecreaseOverload(count map[int]uint64, ratio float64, window uint64) bool {
	step := int64(float64(manager.OverloadControlLevel) * ratio)
	...
	manager.Status = Normal
	log.Warning("Decrease Overload to Normal")
	return false
}

func (manager *WorkEngineManager) RecoverFromOverload() {
	var step int64 = manager.GetTotalConfigLevel() / manager.IdleRecoverRatio
	...
	manager.Status = Normal
	log.Warning("Recover from Overload to Normal")
}

func (manager *WorkEngineManager) PushProcessRequest(task Task) bool {
	if len(manager.ProcessRequests) >= cap(manager.ProcessRequests) {
		log.Debug("Fail to push process request because queue is full")
		return false
	}
	manager.ProcessRequests <- task
	return true
}

func (manager *WorkEngineManager) HandleProcessRequest() {
	for {
		select {
		case task := <-manager.ProcessRequests:

			enterTime := task.EnterTime()
			arrivalTime := time.Now()
			waitTime := arrivalTime.Sub(enterTime).Nanoseconds()

			if manager.Status == Overload && float64(waitTime)/1000000.0 >= manager.OverloadControlLatencyThreshold {
				...
				if manager.PushDeniedRequest(deniedTask) == false {
			...
			latency.GroupPriority = task.GourpPriority()
			latency.Count = 1
			manager.PushTrafficLatency(latency)
		}
	}
}
...
func (manager *WorkEngineManager) PushDeniedRequest(task Task) bool {
	if len(manager.DeniedRequests) >= cap(manager.DeniedRequests) {
		log.Debug("Fail to push denied request because queue is full")
...
func (manager *WorkEngineManager) ReportCounterForNFMgm(totalCounterName, unprocessedCounterName string) {
	...
	for {
		currentUnprocessed = atomic.LoadUint64(&manager.DeniedRequestNumber)

		delta1 := float64(currentUnprocessed - previousUnprocessed)
		if delta1 > 0 {
			pm.Add(delta1, totalCounterName, constvalue.NfManagement, constvalue.MetricWildcard, constvalue.MetricWildcard)
			pm.Add(delta1, unprocessedCounterName, constvalue.NfManagement, constvalue.MetricWildcard, constvalue.MetricWildcard, "503", "NF_CONGESTION")
