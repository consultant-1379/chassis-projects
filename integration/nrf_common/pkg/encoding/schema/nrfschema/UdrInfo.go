...
func (u *TUdrInfo) IsEqual(c TNfInfo) bool {

	b := c.(*TUdrInfo)

	if len(u.ExternalGroupIdentifiersRanges) != len(b.ExternalGroupIdentifiersRanges) {
		return false
	}
	...
	if len(u.SupportedDataSets) != len(b.SupportedDataSets) || u.GroupId != b.GroupId {
		return false
	}
	...
	for k, item := range u.SupportedDataSets {
		if item != b.SupportedDataSets[k] {
			return false
		}
	}
