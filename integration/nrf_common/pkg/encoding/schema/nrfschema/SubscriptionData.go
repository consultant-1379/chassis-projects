...
func (s *TSubscriptionData) ValidateSubscrCond() *problemdetails.ProblemDetails {
	...
	if s.SubscrCond != nil {
		...
		match := false
		nrfPolicy := cm.GetNRFPolicy()
		for _, allowedSubscriptionAllNF := range nrfPolicy.ManagementService.Subscription.AllowedSubscriptionAllNFs {
...
func (s *TSubscriptionData) IsLocalPlmn() bool {
	...
	for _, plmn := range nfProfile.PlmnID {
