...
func (a *TAusfInfo) IsEqual(c TNfInfo) bool {

	b := c.(*TAusfInfo)

	if a.GroupId != b.GroupId || len(a.SupiRanges) != len(b.SupiRanges) {
		return false
	}

	if len(a.RoutingIndicators) != len(b.RoutingIndicators) {
		return false
	}

	for k, item := range a.RoutingIndicators {
		if item != b.RoutingIndicators[k] {
			return false
		}
	}

	for k, item := range a.SupiRanges {
		bb := b.SupiRanges[k]
		if !item.IsEqual(&bb) {
			return false
		}
