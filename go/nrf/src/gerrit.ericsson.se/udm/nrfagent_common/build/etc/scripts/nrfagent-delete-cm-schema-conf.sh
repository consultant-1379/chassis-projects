...
retCode=`curl -i -s -X DELETE   -H "Content-Type:application/json"  ${CM_SERVICE}/configurations/${CM_LOG_NAME}|grep -E "200 OK|404 NOT FOUND"` 
    echo "retCode: $retCode"  
if [[ "${retCode}" =~ "200" ]]; then
    echo "Deleted cm configuration ${CM_LOG_NAME} successfully"
elif [[ "${retCode}" =~ "404" ]];then
    echo "Cannot find cm configuration ${CM_LOG_NAME}"
   exit 0		
else
    echo "delete cm configuration log failure"
#    exit 1
fi
...
retCode=`curl -i -s -X DELETE   -H "Content-Type:application/json"  ${CM_SERVICE}/schemas/${CM_LOG_NAME}|grep -E "200 OK|404 NOT FOUND"` 
    echo "retCode: $retCode"  
if [[ "${retCode}" =~ "200" ]]; then
    echo "Deleted cm schema ${CM_LOG_NAME} successfully"
elif [[ "${retCode}" =~ "404" ]];then
    echo "Cannot find cm schema ${CM_LOG_NAME}"
   exit 0		
else
    echo "delete cm schema log failure"
#    exit 1
fi
