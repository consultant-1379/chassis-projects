...
func QueryWithFilter(queryReq *dbproxy.QueryRequest) (*dbproxy.QueryResponse, error) {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				log.Debug("queryWithFilter receive streaming message end")
				break
			}
...
func PutNFProfileWithID(nfInstanceID, nfProfile string) (*nfprofile.NFProfilePutResponse, error) {
	putReq := &nfprofile.NFProfilePutRequest{NfInstanceId: nfInstanceID, NfProfile: nfProfile}
...
func Insert(region, key, value string) error {
	...
	if insertRespone.Code != DbPutSuccess {
                return fmt.Errorf("Fail to insert data to DB, error code %d", insertRespone.Code)
        }
...
func Remove(region string, key []string) error {
	...
	removeResponse, err := dbclient.Remove(ctx, removeRequest)
        ...
        if removeResponse.Code != DbDeleteSuccess {
                return fmt.Errorf("Fail to remove data from DB, error code %d", removeResponse.Code)
        }
