...
public class FragmentUtil {
    ...
    public static boolean isNeedFragment(String region_name, SelectResults<Object> searchResults) {
        ...
        logger.trace("total transmit size=" + totalTransmitSize + ",Max transmit message size=" + maxTransmitFragmentSize);
    ...
    public static int transmitNumPerTime(FragmentResult fragmentResult, String region_name) {
        int maxTransmitFragmentSize = getMaxTransmitFragmentSize();
        int transmitNum = 0;
        int transmitSize = 0;
        int transmittedNumber = fragmentResult.getTransmittedNumber();
        List<Object> restFragment = fragmentResult.getItems().subList(transmittedNumber, fragmentResult.getItems().size());
        switch (region_name) {
            case Code.NFPROFILE_INDICE:
        ...
        logger.trace("Transmit profile num =" + transmitNum);
        return transmitNum;
    }
    ...
    public static List<QueryResponse> getFragmentResponse(int code, boolean traceEnabled, TraceInfo traceInfo, List<String> response) {
        int maxTransmitFragmentSize = getMaxTransmitFragmentSize();
        int transmitStart = 0;
        int transmitNum = 0;
        ...
        for (int i = 0; i < response.size(); i++) {
            if (transmitMsgSize >= maxTransmitFragmentSize) {
                ...
                if (traceEnabled) {
                    queryResponse = QueryResponse.newBuilder().setCode(code).addAllValue(fragmentList).setTraceInfo(traceInfo).build();
                } else {
                    queryResponse = QueryResponse.newBuilder().setCode(code).addAllValue(fragmentList).build();
                }
