...
{
   if [[ -t 1 && $(which tput) ]]; then
      echo -e "\E[0;32m$1$(tput sgr0)" 2>/dev/null # in green
   else
      echo $1
   fi
}
...
{
   if [[ -t 1 && $(which tput) ]]; then
      echo -e "\E[1;33m$1$(tput sgr0)" 2>/dev/null # in yellow
   else
      echo $1
   fi
}
...
{
   export CHASSIS_PROJECTS_ROOT="$(git rev-parse --show-toplevel 2> /dev/null)"
}
...
###############################################################################
# Returns branch name, commit id in case of detached state, or tag identifier #
# if commit id is associated to any; summing up:                              #
